...
const lines = (before: string, after: string): string[] =>
  plain(renderDiff(before, after, '')).split('\n').map((line) => line.trim());

test('identical leading lines are hidden so the real change is visible', () => {
  const shared = '/** A long shared comment line that would otherwise dominate. */';
...
test('identical trailing lines are hidden too', () => {
  const out = lines('a\nchanged\nz', 'a\nCHANGED\nz');
  assert.deepEqual(out, ['- changed', '+ CHANGED', '(2 unchanged lines hidden)']);
});
...
test('a single line with a long shared prefix is trimmed to the difference', () => {
  ...
  assert.ok((out[0]?.length ?? 0) < 30, 'should be much shorter than the original');
});
...
test('a pure deletion shows only removed lines', () => {
  ...
  assert.equal(out.some((line) => line.startsWith('+')), false);
});
...
test('long runs of lines collapse with an accurate count', () => {
  const after = ['start', ...Array.from({ length: 10 }, (_, i) => `line${i}`), 'end'];
...
test('no unchanged-lines note when everything differs', () => {
  const out = lines('a', 'b');
  assert.equal(out.some((line) => line.includes('unchanged')), false);
});
